...
/** A known Herdr command failure, without echoing arbitrary output into logs. */
export type HerdrCliFailed = { readonly _tag: "HerdrCliFailed"; readonly message: string };

/** Limits for an owned CLI child. */
export type CliOptions = { readonly timeoutMs?: number; readonly maxOutputBytes?: number; readonly signal?: AbortSignal };

function failed(detail: string): HerdrCliFailed {
  return { _tag: "HerdrCliFailed", message: `Herdr CLI: ${detail}` };
}

/** Execute argv directly, bounding runtime/output and killing only the child this call owns. */
export function runHerdr(bin: string, args: ReadonlyArray<string>, options: CliOptions = {}): Promise<Result<string, HerdrCliFailed>> {
...
/** Request a toast through the running Herdr binary; no shell interpolation is used. */
...
/** Open the manifest's stats popup in this invocation's Herdr session. */

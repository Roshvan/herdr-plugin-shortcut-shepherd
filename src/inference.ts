...
type AncestorFocus = "workspace" | "tab";
type Stamped = { readonly event: HerdrEvent; readonly at: number; readonly focusTransition: AncestorFocus | undefined };
...
export type InferenceState = {
  ...
  readonly observedFocus: ObservedFocus;
...
export function createInference(): InferenceState {
  return {
    ...
    observedFocus: { workspaceId: undefined, tabsByWorkspace: new Map() },
...
function loadSnapshot(state: InferenceState, snapshot: SessionSnapshot): void {
  ...
  state.layouts.clear();
  state.observedFocus.tabsByWorkspace.clear();
  for (const ws of snapshot.workspaces.toSorted((a, b) => a.number - b.number)) {
    ...
    state.observedFocus.tabsByWorkspace.set(ws.workspaceId, ws.activeTabId);
  }
  for (const tab of snapshot.tabs) tabList(state, tab.workspaceId).push(tab.tabId);
  ...
  state.observedFocus.workspaceId = snapshot.focusedWorkspaceId;
...
export function observe(state: InferenceState, event: HerdrEvent, at: number): void {
  ...
  const entry = { event, at, focusTransition: observeFocus(state.observedFocus, event) };
  state.pending.push(entry);
  state.recent.push(entry);
}

function observeFocus(focus: ObservedFocus, event: HerdrEvent): AncestorFocus | undefined {
  switch (event.type) {
    case "workspace_focused": {
      const changed = focus.workspaceId !== event.workspaceId;
      focus.workspaceId = event.workspaceId;
      return changed ? "workspace" : undefined;
    }
    case "tab_focused": {
      const changed = focus.tabsByWorkspace.get(event.workspaceId) !== event.tabId;
      focus.tabsByWorkspace.set(event.workspaceId, event.tabId);
      return changed ? "tab" : undefined;
    }
    case "workspace_created":
      focus.tabsByWorkspace.set(event.workspace.workspaceId, event.workspace.activeTabId);
      return undefined;
    case "workspace_closed":
      focus.tabsByWorkspace.delete(event.workspaceId);
      if (focus.workspaceId === event.workspaceId) focus.workspaceId = undefined;
      return undefined;
    default:
      return undefined;
  }
...
function nearFocusTransition(state: InferenceState, at: number, ancestors: ReadonlyArray<AncestorFocus>): boolean {
  return state.recent.some(
    (entry) => Math.abs(entry.at - at) <= INFERENCE.focusSideEffectMs
      && entry.focusTransition !== undefined && ancestors.includes(entry.focusTransition),
  );
}
...
function evaluateWorkspaceFocused(state: InferenceState, workspaceId: WorkspaceId, at: number): Detection | undefined {
  ...
  const movedOnAgain = state.recent.some(
    (entry) => entry.at > at && entry.at - at <= INFERENCE.workspaceDwellMs && entry.focusTransition === "workspace",
  );
...
function evaluateTabFocused(state: InferenceState, tabId: TabId, workspaceId: WorkspaceId, at: number): Detection | undefined {
  ...
  if (nearFocusTransition(state, at, ["workspace"])) return undefined;
  if (near(state, at, INFERENCE.focusSideEffectMs, ["tab_moved", "pane_moved"], () => true)) return undefined;
...
function isFocusSideEffect(state: InferenceState, at: number, tabId: TabId, workspaceId: WorkspaceId): boolean {
  return nearTabStructure(state, at, tabId, workspaceId)
    || nearFocusTransition(state, at, ["workspace", "tab"])
    || near(state, at, INFERENCE.focusSideEffectMs, FOCUS_SIDE_EFFECT_EVENTS, () => true);
}
...
function applyTabEvent(state: InferenceState, event: HerdrEvent): void {
  switch (event.type) {
    case "tab_created": {
      ...
      if (!tabs.includes(event.tab.tabId)) tabs.push(event.tab.tabId);
      return;
    }
    case "tab_closed": {
    ...
    case "tab_moved":
      state.tabsByWorkspace.set(event.workspaceId, event.tabs.map((tab) => tab.tabId));
      return;
    case "tab_focused":
...
function directionBetween(from: Rect, to: Rect): HerdrActionId | undefined {
